      ...
      >
         {branches?.map((branch, index) => (
            <Card key={index} className="hover:shadow-xl transition-all">
               <CardHeader className="bg-yellow-500">{branch.name}</CardHeader>
               <CardBody className="h-[350px] overflow-auto">
                  {branch.note || "برای مشاهده منوی این شعبه کلیک کنید"}
               </CardBody>
               <CardFooter className="flex justify-center">
                  <Link href={`/branch/${branch.id}`}>
                     <Button className="bg-gradient-to-r from-[#9c614b] to-[#9c614b] text-slate-100">
                        مشاهده منوی شعبه
                     </Button>
                  </Link>
               </CardFooter>
